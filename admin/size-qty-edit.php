...
if (isset($_POST['form1'])) {
	...
	if (empty($_POST['color_hex'])) {
		$valid = 0;
		$error_message .= "Color Hex Code can not be empty<br>";
	} else {
		// Check if color hex code is valid
		if (!preg_match('/^#[a-fA-F0-9]{6}$/', $_POST['color_hex'])) {
			$valid = 0;
			$error_message .= "Invalid Color Hex Code<br>";
		}
	}
	...
	if ($valid == 1) {
		...
		$statement = $pdo->prepare("UPDATE tbl_color SET color_name=?, color_hex=? WHERE color_id=?");
		$statement->execute(array($_POST['color_name'], $_POST['color_hex'], $_REQUEST['id']));
			...
			<form class="form-horizontal" action="" method="post">
				...
				<div class="box box-info">
					...
					<div class="box-body">
						...
						<div class="form-group">
							<label class="col-sm-2 control-label" for="color_hex">Color Hex Code:</label>
							<div class="col-sm-10">
								<input class="form-control" type="color" name="color_hex" id="color_hex" value="<?php echo $hex_color; ?>" required>
							</div>
						</div>
